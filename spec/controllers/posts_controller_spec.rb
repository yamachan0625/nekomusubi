...
describe PostsController do
  ...
  describe '#update' do
    let!(:post) { create(:post, user_id: user.id) }
    let!(:params) { attributes_for(:post, title: "new_title", user_id: user.id) }
    context 'ログインしている時' do
      ...
      it "@postを割り当てる" do
        patch :update, params: {id: post.id, post: params}
        expect(assigns(:post)).to match(post)
      end
      ...
      context '更新に成功した時' do
        ...
        it '同じpostが更新される' do
          expect{patch :update, params: {id: post.id, post: params}}.to change(Post, :count).by( 0 )
        end
